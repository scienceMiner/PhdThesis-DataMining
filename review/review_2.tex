...
\begin{displaymath}
agr(r) = \{ X \mid \quad\mbox{for all distinct tuples}\quad t_1,t_2 \in r,
ag(t_1,t_2) =  X \}
\end{displaymath}
\end{rm}
\end{definition}
...
a set of FDs.  
These are (1) {\bf Disjoint Union } Create an isomorphic copy of each
...
$\sigma$. Then form the union for all {\em standard} FDs to give an
AR. It will not work for nonstandard FDs. (2) Create {\bf Agreement
...
product of these is then formed. (4) Use {\bf Chase Procedures} Given
an Armstrong database which obeys the FD set
...
constants to be added to the database. (5) {\bf Random Relations} shown not to work. We shall see in
...
where each non-trivial FD has a superkey as a $lhs$. BCNF attempts to
overcome the deficiencies in 3NF which may arise in the case  where
there are candidate keys, superkeys without subsets $X$ such that such
...
not constraint preserving.  A non-mathematical treatment of normal
...
Mannila and R\"{a}ih\"{a} , \cite{mr86,mr92}, approached various database
...
to tuple and equality generating dependencies in \cite{bv84}. 
...
specialised for these dependencies and databases. Additionally, the
...
approximates F in chapter~\ref{chap:consistency}. The probability of
...
Chapter~\ref{chap:tempresult}. Numerical Dependencies were themselves generalised to branching
...
is a statement of the form $A \stackrel{(p,q)}{\rightarrow} B$ which
...
this is equivalent to a functional dependency. The following lemma is a
...
implying the semantics of this definition, and we define {\em mean
...
set of functional dependencies $F$ that hold for a database. In
...
\to B$. Note that in Table~\ref{tbl:noncon1} the ND $A \to^2 B$ is
satisfied.

\smallskip

In the context of this thesis we
refer to OR-objects as indefinite cells; this removes the need to
...
We now move on to Temporal Databases and Temporal Data Dependencies.
...
dependencies into the relational model wich constrain the evolution of
a database. We
...
temporal data models. We remark that the
temporal dependencies of \cite{Via87,wij95} may be mined similarly
to standard FDs.
...
granularity and lifespan. A temporal data value is defined as a
...
also denotes the three main aims of time series analysis as being:
 (1) {\bf Forecasting } which attempts to predict short term system
evolution. (2) {\bf Modelling} which attempts to describe long term
system behaviour, and (3) {\bf Characterisation} which attempts to
...
making FD inference unfeasible.
...
\to I$ is satisfied functionally or close to functionally. We therefore
...
be invalid in the database as well as the valid dependencies. Recently
...
determinations and various constraint satisfaction procedures.  \cite{bb95} discusses the problem of dependency inference within real world databases where a set oriented interface is detailed. Machine learning can be said to be the inference of general rules from instances of data and, as such, dependency inference is an attractive branch given that for the instances of data, the database itself, there always exists a concept which fits the data set, namely the dependency set \cite{mr94}.
...
In \cite{km95} the problem described is to find the set of functional
dependencies in a {\em cover} $r$ where the set of FDs for r, where r
is a minimal set, are equivalent to the set of FDs for the relations
in R, the set for the whole database.   Naive algorithms to do this
...
hypergraph transversals, or hitting sets. \cite{mr92} state that it is
...
viewed as a collection of weighted subsets. We present some results
...
series. It is not concerned with how or why this is achieved. Alternatively,
...
episode rule is a frequency of each episode occurring
...
Mannila and Toivonen state that finding whether a serial or parallel
...
\caption{\label{graph:simquality}{Max Quality for an FD set of size 3}}
\end{figure}
...
these variations for a fixed FD set of size 3.
...
The similarity measure is monotonically increasing and if a {\em core} of the two FD sets is
...
form cannot form any part of the core of $G$ and $H$. Using
...
presented an overview of a similarity measure for FD sets. We show in
chapter~\ref{chap:numdep} that a finite axiomatisation of NDs does not
exist and so we do not consider closure of ND sets for a measure and
define a metric based on the lattice properties of NDs. 
