...
\sa.  \> \> \> \> \> Rule\_set := $\{ p \mid $ property $p$ discovered from
...
\sa.  \> \> \> \> {\bf end if};\\
\sa.  \> \> \> \> {\bf if } $\exists$ r $\in$ Rule\_set and $\exists$ r$_2$
$\in$ Rule\_set with r $\not=$ r$_2$ \\
...
\pers{10}{5} ($HOME\_FAV$ $\uparrow_r \wedge^0$ $FAV\_TEAM\_WIN$
...
\resp{80}{20} ($ag \uparrow \wedge^{0} db \uparrow$ ) and  \resp{80}{20}
...
found \pers{30}{15} ($\downarrow \wedge^{0}\downarrow$ ) became
\pers{30}{15} ($\downarrow \wedge^{3}\downarrow$ ) when the overlap
